...
impl ProxyMonitor {
    ...
    pub async fn log_request(&self, log: ProxyRequestLog) {
        ...
        tokio::spawn(async move {
            ...
            // [NEW] POST Usage Log to Control Plane for unified billing
            let mut user_id = log_to_save.username.clone().unwrap_or_default();
            let mut api_key_id = String::new();
            if let Some(idx) = user_id.find("::") {
                api_key_id = user_id[idx+2..].to_string();
                user_id = user_id[0..idx].to_string();
            }
            let client = reqwest::Client::new();
            let cp_log = serde_json::json!({
                "id": log_to_save.id.clone(),
                "api_key_id": api_key_id,
                "user_id": user_id,
                "model_alias": log_to_save.model.clone().unwrap_or_default(),
                "provider": log_to_save.protocol.unwrap_or_else(|| "native".to_string()),
                "account_id": log_to_save.account_email.clone().unwrap_or_default(),
                "upstream_model": log_to_save.mapped_model.clone().unwrap_or_default(),
                "status": if log_to_save.error.is_some() { "error" } else { "success" },
                "error_message": log_to_save.error.clone().unwrap_or_default(),
                "upstream_status_code": log_to_save.status,
                "prompt_tokens": log_to_save.input_tokens.unwrap_or(0),
                "completion_tokens": log_to_save.output_tokens.unwrap_or(0),
                "total_tokens": log_to_save.input_tokens.unwrap_or(0) + log_to_save.output_tokens.unwrap_or(0)
            });
            let _ = client.post("http://127.0.0.1:8080/api/internal/usage-logs").json(&cp_log).send().await;
        });
