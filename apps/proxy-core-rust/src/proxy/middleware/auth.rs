...
async fn auth_middleware_internal(
    ...
    // 认证逻辑: Query Control Plane
    let mut authorized = false;
    let mut identity_ext = None;
    
    if force_strict {
        match &security.admin_password {
            Some(pwd) if !pwd.is_empty() => {
                authorized = api_key.as_deref().map(|k| k == pwd).unwrap_or(false);
            }
            _ => {
                authorized = api_key.as_deref().map(|k| k == security.api_key).unwrap_or(false);
            }
        }
    } else {
        if let Some(ak) = &api_key {
            // Check hardcoded proxy key first as fallback fallback
            if ak == &security.api_key {
                authorized = true;
                ...
                let payload = serde_json::json!({ "key": ak });
                if let Ok(resp) = client.post("http://127.0.0.1:8080/api/internal/validate-key").json(&payload).send().await {
                    if resp.status().is_success() {
                        ...
                        if let Ok(json) = resp.json::<serde_json::Value>().await {
                            identity_ext = Some(UserTokenIdentity {
                                ...
                                total_quota: json["total_quota"].as_f64().unwrap_or(0.0),
                                group_id: json["user_group"].as_str().unwrap_or("").to_string(),
                            });
                        }
                    }
                }
    ...
    if authorized {
        if let Some(identity) = identity_ext {
            let (mut parts, body) = request.into_parts();
            parts.extensions.insert(identity);
            let request = Request::from_parts(parts, body);
