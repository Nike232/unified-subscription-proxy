...
pub struct AppState {
    ...
    pub rate_limiter: Arc<crate::proxy::middleware::rate_limit::RateLimiter>, // [NEW] 速率限制器
}
...
impl AxumServer {
    ...
    proxy_pool_manager.clone().start_health_check_loop();
        ...
        let rate_limiter = Arc::new(crate::proxy::middleware::rate_limit::RateLimiter::new());
        ...
        let proxy_routes = Router::new()
            ...
            // 请求: ip_filter -> auth -> rate_limit -> monitor -> handler
            // 响应: handler -> monitor -> rate_limit -> auth -> ip_filter
            ...
            .layer(axum::middleware::from_fn_with_state(
                state.clone(),
                crate::proxy::middleware::rate_limit::rate_limit_middleware,
            ))
            .layer(axum::middleware::from_fn_with_state(
                state.clone(),
                auth_middleware,
